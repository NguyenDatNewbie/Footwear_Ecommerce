...
<body>
    ...
    <div class="overlay" id="overlay">
        <div class="overlay-content" style="margin-top: 20px; display: inline-block;">
            ...
            <img style="width: 700px; height: 600px;" src="https://www.gento.vn/wp-content/uploads/2022/05/bang-size-giay-nike-nam-e1654001239325.jpg"/>
        </div>
    </div>
                           ...
                           <div class=" product_ratting" style="margin-bottom: 0px">
                                    <ul>
                                        <c:forEach items="${evaluateService.rateStar(rateAvg)}" var="star">
                                            <c:choose>
                                                <c:when test="${star==1}"><li><i class="fa fa-star"></i></li></c:when>
                                                <c:when test="${star==0}"><li><i class="fa fa-star-o" style="color: #faca50;"></i></li></c:when>
                                                <c:otherwise>
                            ...
                            <div class="product_price">
                                <c:choose>
                                    <c:when test="${productCurrent.promotion>0}">
                                        <span style="color:red;" class="current_price" >${formatVND.format(productCurrent.price*(1-productCurrent.promotion/100))}</span>
                                        <span class="old_price">${formatVND.format(productCurrent.price)}</span>
                                    </c:when>
                                    <c:otherwise>
                                        <span style="color:red;" class="current_price">${formatVND.format(productCurrent.price)}</span>
                                    </c:otherwise>
                                </c:choose>
                            </div>
                                ...
                                <div class="review">
                                            ...
                                            <div class="comment item" style="margin-bottom: 20px">
                                                <div class="comment-rate" style="display: flex;">
                                                    ...
                                                    <ul style="display: flex; margin-right: 15px">
                                                        <c:forEach items="${evaluateService.rateStar(evaluate.rate)}" var="star">
                                                            <c:choose>
                                                                <c:when test="${star==1}"><li><i class="fa fa-star"></i></li></c:when>
                                                                <c:when test="${star==0}"><li><i class="fa fa-star-o" style="color: #faca50;"></i></li></c:when>
                                                                <c:otherwise>
                                    ...
                                    <div class="review_summary">
                                        <div class="review_summary_figure" >
                                            <div class="line_figure" style="margin-bottom: 3px"><strong style="color: #faca50">${Math.round(evaluateService.avg(evaluateList)*100.0)/100.0}</strong><strong>/5</strong></div>
                                        ...
                                        <div class="review_summary_detail" style="display: flex; justify-content: center">
                                            <div class="start_static">
                                                <c:forEach items="${evaluateService.createListInt(5)}" var="countStar">
                                                    <div class="rate_avg_star">
                                                        <ul style="display: flex;">
                                                            <c:forEach items="${evaluateService.rateStar(countStar)}" var="star">
                                                                <c:choose>
                                                                    <c:when test="${star==1}"><li><i class="fa fa-star"></i></li></c:when>
                                                                    <c:when test="${star==0}"><li><i class="fa fa-star-o" style="color: #faca50;"></i></li></c:when>
                                                                    <c:otherwise>
                                            ...
                                            <div class="rate_count">
                                                <c:forEach items="${evaluateService.createListInt(5)}" var="rateValue">
                                                    <div>
                                                        <c:set var="count" value="${evaluateRepository.countRate(productCurrent.id,rateValue)}"/>
                                                        <progress max="${evaluateList.size()}" value="${count}" aria-busy=""></progress>
                                                        <span>${count}</span>
            ...
            <div class="product_area">
                <div class="row">
                    <div class="product_carousel product_three_column4 owl-carousel">
                        <c:forEach items="${productService.findAllByProductCategorySoldTop(productCurrent.category.id)}" var="product">
                            <div class="col-lg-3">
                                <div class="single_product">
                                    <div class="product_thumb">
                                        <c:if test="${imageRepository.findAllByProduct(product.id).size()>0}">
                                            <a class="primary_img" href="product-details.jsp"><img src="${imageRepository.findAllByProduct(product.id).get(0).img}" alt=""></a>
                                        </c:if>
                                        <c:if test="${imageRepository.findAllByProduct(product.id).size()>1}">
                                            <a class="secondary_img" href="product-details.jsp"><img src="${imageRepository.findAllByProduct(product.id).get(0).img}" alt=""></a>
                                        </c:if>

                                        <div class="quick_button">
                                            <a href="#" data-toggle="modal" data-target="#modal_box-${product.id}" title="quick_view">Xem nhanh</a>
                                        </div>
    ...
    <!-- modal area start-->
    <c:forEach var="product" items="${productService.addAll(productService.findAllByProductCategorySoldTop(productCurrent.id),productService.findNewProduct(10)) }">
        <!-- modal area start-->
                                    ...
                                    <div class="col-lg-5 col-md-5">
                                        <div class="product-details-tab">
                                            <div id="img-2" class="zoomWrapper single-zoom">
                                                <img id="zoom2" src="${imageRepository.findAllByProduct(product.id).get(0).img}"
                                        ...
                                        <div class="product_d_right">
                                            <form action="#">
                                                ...
                                                <c:if test="${evaluates.size()>0}">
                                                    <div class=" product_ratting" style="margin-bottom: 0px">
                                                        <ul>
                                                            <c:forEach items="${evaluateService.rateStar(evaluateService.rateAvg(evaluates))}" var="star">
                                                                <c:choose>
                                                                    <c:when test="${star==1}"><li><i class="fa fa-star"></i></li></c:when>
                                                                    <c:when test="${star==0}"><li><i class="fa fa-star-o" style="color: #faca50;"></i></li></c:when>
                                                                    <c:otherwise>
                                                ...
                                                <div class="product_price">
                                                    <c:choose>
                                                        <c:when test="${product.promotion>0}">
                                                            <span style="color:red;" class="current_price" >${formatVND.format(product.price*(1-product.promotion/100))}</span>
                                                            <span class="old_price">${formatVND.format(product.price)}</span>
                                                        </c:when>
                                                        <c:otherwise>
                                                            <span style="color:red;" class="current_price">${formatVND.format(product.price)}</span>
                                                        </c:otherwise>
                                                    </c:choose>
                                                </div>
                                                ...
                                                <div class="product_variant size" style="display: block; margin-bottom: 10px">
                                                    ...
                                                    <div class="size_product" style="width: 40%">
                                                        <c:forEach var="size" items="${sizeRepository.findAllByProductId(product.id)}">
                                                            <div class="box_size" onclick="changeSize(this)">${size.size}</div>
                                                        </c:forEach>
                                                    </div>
                                                </div>
